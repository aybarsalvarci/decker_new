            ...
            <div class="card card-outline-danger shadow-sm">
                ...
                <div class="card-header bg-white py-3">
                    <h3 class="card-title font-weight-bold">
                        <i class="fas fa-file-pdf mr-1 text-danger"></i> Manage Warranties Page
                    </h3>
                </div>

                <form action="{{route('admin.resources.warranties')}}" method="post" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')

                    <div class="card-body p-4">
                        @if(isset($page) && $page->file)
                            <div class="form-group">
                                <label>Current Active Catalog:</label>
                                <div class="current-file-box">
                                    <i class="fas fa-file-pdf pdf-icon-large"></i>
                                    <div style="flex-grow: 1;">
                                        <h5 class="m-0 font-weight-bold">{{ $page->title_en ?? 'Online Catalog' }}</h5>
                                        <small class="text-muted">Uploaded: {{ $page->updated_at->format('d.m.Y H:i') }}</small>
                                    </div>
                                    <a href="{{ asset('storage/' . $page->file) }}" target="_blank" class="btn btn-sm btn-outline-danger">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label for="title_es">Page Title Title (Spanish)</label>
                                    <div class="input-group">
                                        <div class="input-group-prepend">
                                        ...
                                        <input type="text" name="title_esp" id="title_es"
                                               class="form-control"
                                               value="{{ old('title_es', $page->title_esp) }}"
